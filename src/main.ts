...
function boot(): void {
  ...
  bundle.canvas.setAttribute(
    'aria-label',
    '일몰 상영관 3D 공간. 이동은 WASD 또는 방향키, 시점은 마우스 또는 Q E R F, 일시정지는 Esc.',
  );
  ...
  // Positional guides: screen source audible from the corridor (leads the walk),
  ...
  store.subscribe(
    ...
    (q) => {
      ...
      lastQuality = q;
      qc.setManual(q);
      // Manual preset change may change rendition (SRS-VID-1 / §6.5)
      setTimeout(() => {
  ...
  void qc.detect(store.get().settings.quality).then(() => {
    // initial rendition target (applies when screen loads)
    const r = pickRendition();
    if (r !== screen.currentRendition) void screen.changeRendition(r);
  });
